...
class UserRepository(BaseRepository):
    ...
    async def update_user(self, search_by: dict, user: dict) -> UserDTO | None:
        search_by = DataUtils.filter_search_fields(search_by, self.model)
    ...
    async def get_user_id_by_login(self, login: str, with_soft_deleted: bool = False) -> int | None:
        ...
        if not id:
            ...
            data = RedisSetexRequestDTO(name=result.login, time=getenv("USER_TTL"), value=id)
    ...
    async def get_users(self, search_by: dict, with_relations: bool, with_soft_deleted: bool = False) -> list[UserDTO]:
        ...
        if not search_by and not with_soft_deleted:
            ...
            if not stringified_users_list:
                ...
                stringified_users_list = json.dumps(User.nested_models_to_dict(result), default=str)

                data = RedisSetexRequestDTO(name=key, time=getenv("USER_TTL"), value=stringified_users_list)

                await redis_client.setex(**data.model_dump())

            return [UserDTO(**user_with_relations) for user_with_relations in json.loads(stringified_users_list)]
